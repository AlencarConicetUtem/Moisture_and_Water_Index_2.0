...
from PyQt5.QtWidgets import QAction,QFileDialog, QDialog, QLabel, QMessageBox, QMenu, QSizePolicy, QVBoxLayout
...
class IndiceNDWI:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&Moisture and Water Index 2.0')
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr(u'&Moisture and Water Index 2.0'),
                action)
    ...
    def guardar1(self, ruta_guardar):
        opcion = 'TIF'

        self.opcion_seleccionada.append(opcion)
    ...
    def guardar2(self, ruta_guardar):
        opcion = 'TIF Y ASC'
    ...
    def ventana(self):
        # Crear un menú emergente con opciones
        self.menu = QMenu()
        op1 = self.menu.addAction('Guardar solo en formato *.TIFF')
        op2 = self.menu.addAction('Guardar en formato *.TIFF y *.ASC')

        # Conectar las acciones a la función guardar y guardar la opción seleccionada
        op1.triggered.connect(self.guardar1)
        op2.triggered.connect(self.guardar2)
        
        # Asociar el menú emergente al botón
        self.menu.setGeometry(300, 300, 300, 60)
        self.menu.setWindowTitle('Menú emergente')
        pos = self.dlg.btn_guardar.mapToGlobal(self.dlg.btn_guardar.rect().bottomLeft())
        pos.setX(pos.x() + 25)  # Agregar 50 píxeles a la coordenada x de pos
        self.menu.exec_(pos)
    ...
    def ejecutar(self):
        ...
        if self.opcion_seleccionada[0] == 'TIF':
        ...
        elif self.opcion_seleccionada[0] == 'TIF Y ASC':

            os.mkdir(self.ruta_guardar[0] + '/TIF')
            os.mkdir(self.ruta_guardar[0] + '/ASC')
            ...
            if self.estado_corr == "activo":
                ...
                self.GUARDAR_NDWI(banda_3,banda_6,banda_5)
                self.guardar_ndwi_ascii('/TIF/NDWI_XU.TIF', '/TIF/NDWI_GAO.TIF','/TIF/NDWI_MCFEETERS.TIF')
                self.message_bar.clearWidgets()
                self.message_bar = self.iface.messageBar()
                self.message_bar.pushMessage("ANUNCIO:", "Se generó exitosamente el cálculo del índice NDWI en formato .TIF y .ASC a través de los métodos Xu, Gao y McFeeters.", level=Qgis.Success, duration=0)
                self.limpiar()
    ...
    def limpiar(self):
        ...
        self.opcion_seleccionada.clear()
    ...
    def GUARDAR_NDWI(self,directorio, directorio1, directorio2):
        if self.opcion_seleccionada[0] =='TIF':
            ...
            self.NDWI(directorio, directorio1, "/TIF/NDWI_XU.TIF", 'green@1','swir@1')
            self.NDWI(directorio2, directorio1, "/TIF/NDWI_GAO.TIF", 'nir@1','swir@1')
            self.NDWI(directorio, directorio2, "/TIF/NDWI_MCFEETERS.TIF", 'green@1','nir@1')

            self.iface.addRasterLayer(self.ruta_guardar[0] + '/TIF/NDWI_XU.TIF', "NDWI_XU")
            self.iface.addRasterLayer(self.ruta_guardar[0] + '/TIF/NDWI_GAO.TIF', "NDWI_GAO")
            self.iface.addRasterLayer(self.ruta_guardar[0] + '/TIF/NDWI_MCFEETERS.TIF', "NDWI_MCFEETERS")

        
    def tranformar_ascii(self,ruta_tiff,ruta_guardar_asc):
        # Ruta y nombre del archivo de entrada
        input_file = ruta_tiff
        # Ruta y nombre del archivo de salida
        output_file = self.ruta_guardar[0] + ruta_guardar_asc
        # Abre el archivo de entrada como un dataset
        input_dataset = gdal.Open(input_file)
        # Define las opciones de creación para evitar archivos auxiliares adicionales
        options = ['-of AAIGrid', '-co PROFILE=BASELINE', '-co WRITE_BUNDLE=YES']
        # Utiliza el método gdal.Translate() para escribir el archivo en formato ASCII sin archivos auxiliares adicionales
        gdal.Translate(output_file, input_dataset, creationOptions=options)
        # Cierra el dataset
        input_dataset = None
        
               
    def guardar_ndwi_ascii(self, ruta1, ruta2, ruta3):
        self.tranformar_ascii(self.ruta_guardar[0] + ruta1 ,'/ASC/NDWI_XU.ASC')
        self.tranformar_ascii(self.ruta_guardar[0] + ruta2 ,'/ASC/NDWI_GAO.ASC')
        self.tranformar_ascii(self.ruta_guardar[0] + ruta3 ,'/ASC/NDWI_MCFEETERS.ASC')
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg.btn_guardar.clicked.connect(self.ventana)
